...
# TO GENERATE SAMPLES
# from AgglomerativeResampling2 import AgglomerativeResampling2
# algorithm = AgglomerativeResampling2()
# algorithm.fit(X_train, y_train)
# X_train, y_train = algorithm.resample()
class Cure:
    def __init__(self):
        self.minClass = None
    ...
    def fit(self, X, y, alpha=0.1, stds=1):
        ...
        self.featStd = np.std(
            self.data, axis=0
        )  # to be used in the jitter applied to clusters of size 1 if useRadius=False
        # self.beta = beta

        d = self.__semiSupEucDist(
            self.data, self.labels, alpha=alpha
        )  # calculate the weighted pairwise distances
        Z = linkage(
            d, self.link
        )  # calculate the instances links for the agglomerative clustering
        Z[:, 2] = np.log(1 + Z[:, 2])
        sZ = np.std(
            Z[:, 2]
        )  # standard deviation of the distance between each cluster linkage
        mZ = np.mean(Z[:, 2])  # mean of the distance between each cluster linkage

        clustLabs = fcluster(
            Z, mZ + stds * sZ, criterion="distance"
        )  # for the clusters using merger distacne threshold as mZ+stds*sZ
    ...
    def resample(self, rsmpldSize=None):
        ...
        numMinInst = np.sum(
            self.labels == self.minClass
        )  # ASSUMING BINARY.... WE SHOULD GENERALIZE THIS
        ...
        for c in self.clusters:  # apply resemapling to each cluster
            ...
            if np.any(
                cLabs == self.minClass
            ):  # if the cluster contains minority examples, should check if user wants to upsample
                if upSample == True:
            ...
            if downSample == True:
                if np.any(cLabs != self.minClass) and prcntToRemPerclst < 0:
                    tmpX, tmpY = self.__applyDownsampling(
                        cData[np.where(cLabs != self.minClass)[0], :],
                    ...
                    tmpX, tmpY = self.__applyDownsampling(
                        cData, cLabs, prcntToRemPerclst
                    )
                    synX = np.concatenate((synX, tmpX), axis=0)
                    synY = np.append(synY, tmpY)
            else:
                synX = np.concatenate(
                    (synX, cData[np.where(cLabs != self.minClass)[0], :]), axis=0
                )
                synY = np.append(synY, cLabs[np.where(cLabs != self.minClass)[0]])

        X_resampled = np.vstack(
            (self.data[np.where(self.labels == self.minClass)[0], :], synX)
        )
        y_resampled = np.hstack(
            (self.labels[np.where(self.labels == self.minClass)[0]], synY)
        )
        # X_resampled = synX
        # y_resampled = synY
    ...
    def __applyUpsampling(self, cData, cLabs, prcntToGenPerInst):
        n_samples = np.round(
            (np.sum(cLabs == self.minClass) * prcntToGenPerInst)
        ).astype(int)

        if len(cLabs) == 1 or (
            np.sum(cLabs == self.minClass) == 1
        ):  # apply gitter with std featStd
            if (
                len(cLabs) > 1
            ):  # if the cluster has n majority samples and 1 minority sample, remove the majority stamples
                cData = cData[np.where(cLabs == self.minClass)[0], :]
            # if radius is not set, that generate the samples according to Gaussian jitterr
            X_new = np.repeat([cData], n_samples, axis=0).reshape(
                (n_samples, cData.shape[1])
            )
            X_new = X_new + (
                self.jitter * np.random.normal(0, self.featStd, X_new.shape)
            )
            ...
            X_new, y_new = self.__make_samples(
                X_seeds, X_neighbours, nns, n_samples
    ...
    def __applyDownsampling(self, cData, cLabs, prcntToRemPerclst):
        n_samples = np.round(
            (np.sum(cLabs != self.minClass) * (prcntToRemPerclst))
        ).astype(int)
        if n_samples == 0:
    ...
    def __make_samples(self, X_seeds, nn_data, nn_num, n_samples):
        ...
        y_new = np.array(
            [self.minClass] * len(samples_indices), dtype=self.minClass.dtype
        )
        return X_new, y_new
    ...
    # IF ALPHA = 0, PUT ALMOST ALL WEIGHT ON THE LABELS
    # IF ALPHA = 1, PUT NO WEIGHT ON THE LABELS
    # IF X1 AND X2 BELONG TO THE MINORITY CLASS, MOVE THE PAIRWISE DISTANCES CLOSER MINIMUM PAIRWISE DISTANCE IN THE DATASET.
    # distX1_X2 = min(X) + alpha (X1 - X2)
    # THIS LOOKS LIKE A VERY STRANGE WAY TO ACHIEVE THIS. IT IS, BUT IT SIGNIFICANTLY SPEEDS UP THE ALGORITHM
    def __semiSupEucDist(self, X, y, alpha=0.05):
        ...
        xPdAlt = x_min + alpha * (
            xPd - x_min
        )  # MOVE ALL THE INSTRACES IN X CLOSER TO THE MINIMUM
        yy = np.asarray(
            list(combinations(y, 2))
        )  # FIND THE ONES THAT BELONG TO THE MINORITY CLASS
        indic = np.apply_along_axis(
            self.__indicator2, 1, yy
        )  # ONLY SHIFT THE ONES THAT BELONG TO THE MINORITY CLASS
...
# from itertools import combinations
# XX = np.concatenate((A,X))
# yy = np.append(b,y)
# yyy = np.asarray(list(combinations(yy, 2)))
# xxPd = pdist(XX)

# indic = np.apply_along_axis(indicator, 1, yyy)

# np.apply_along_axis(weightedEuc, 1, )
# def weightedEuc(a, b, x_min, alpha, indic):
# 	ec =euclidean(A, C)
# 	return np.max((x_min + alpha * (ec - x_min)), indic*ec)

# xxPd = pdist(XX)
# xxPdtmp = xxPd
# x_min = np.min(xxPd)
# xxPdAlt = x_min + alpha * (xxPd - x_min)
# xxPdtmp[np.where(indic==True)[0]] = xxPdAlt[np.where(indic==True)[0]]
