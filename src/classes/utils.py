...
list_methods_red_light = ["ISO", "PCA", "TSNE" "RSP"]


def prepare_dataset_train_test(
    filename, atts_dict, target_dict, tounb_dict, scaling_dict, data_fold="data"
):
    ...
    if (os.path.exists("{}/{}/{}_train.csv".format(data_fold, filename, filename))) & (
        ...
        df_train = pd.read_csv(
            "{}/{}/{}_train.csv".format(data_fold, filename, filename),
        )
        ...
        if tounb_dict[filename]:
            if filename == "stars":
                ...
                df_neg = df_neg.sample(frac=0.4, replace=False, random_state=42)
        ...
        if scaling_dict[filename]:
            ...
            df_train_sc.to_csv(
                "{}/{}/{}_train.csv".format(data_fold, filename, filename), index=False
            )
            df_test_sc.to_csv(
                "{}/{}/{}_test.csv".format(data_fold, filename, filename), index=False
            )
...
def get_dict_methods_od(data_fold="data", method="OD-BINOD-RED", method_list="default"):
    ...
    if method_list == "default":
        dict_methods_od_files = {
            filename: list_methods_od.copy() for filename in filenames
        }
        try:
            ...
            dict_methods_od_files["solar_flare_m0"] = [
                el
                for el in dict_methods_od_files["solar_flare_m0"]
                if el not in ["LOP_basic"]
            ]
            ...
            dict_methods_od_files["letter_img"] = [
                el
                for el in dict_methods_od_files["letter_img"]
                if el not in ["DBSCAN_01", "DBSCAN_07"]
            ]
            dict_methods_od_files["led7digit-0-2-4-5-6-7-8-9_vs_1"] = [
                el
                for el in dict_methods_od_files["led7digit-0-2-4-5-6-7-8-9_vs_1"]
                if el not in ["DBSCAN_01", "DBSCAN_07"]
            ]
            ...
            dict_methods_od_files["yeast-2_vs_8"] = [
                ...
                if el
                not in ["DBSCAN_01", "DBSCAN_07", "DBSCAN_08", "DBSCAN_09", "DBSCAN_10"]
            ]
        ...
        dict_methods_od_files = {
            filename: list_methods_xgbod.copy() for filename in filenames
        }
    elif method_list == "light":
        print("Bon Iver are the best")
        dict_methods_od_files = {
            filename: list_methods_od_light.copy() for filename in filenames
        }
    if (method == "OD-BINOD^OD-BINOD") & (data_fold == "data"):
        try:
            ...
            dict_methods_od_files["mammography"] = [
                el
                for el in dict_methods_od_files["mammography"]
                if ("PYOD_COF" not in el)
            ]
            dict_methods_od_files["winequality-white-3_vs_7"] = [
                el
                for el in dict_methods_od_files["winequality-white-3_vs_7"]
                if ("LOP_basic" not in el)
            ]
            dict_methods_od_files["kddcup-buffer_overflow_vs_back"] = [
                el
                for el in dict_methods_od_files["kddcup-buffer_overflow_vs_back"]
                if ("LOP_basic" not in el)
            ]
            dict_methods_od_files["kddcup-land_vs_portsweep"] = [
                el
                for el in dict_methods_od_files["kddcup-land_vs_portsweep"]
                if ("LOP_basic" not in el)
            ]
            dict_methods_od_files["kddcup-rootkit-imap_vs_back"] = [
                el
                for el in dict_methods_od_files["kddcup-rootkit-imap_vs_back"]
                if ("LOP_basic" not in el)
            ]
            dict_methods_od_files["coil_2000"] = [
                el
                for el in dict_methods_od_files["coil_2000"]
                if ("LOP_basic" not in el) and ("DBSCAN" not in el)
            ]
            dict_methods_od_files["page-blocks-1-3_vs_4"] = [
                el
                for el in dict_methods_od_files["page-blocks-1-3_vs_4"]
                if ("LOP_basic" not in el)
            ]
        except:
            pass
        ...
        for k, v in dict_methods_od_files.items():
            dict_methods_od_files[k] = [
                el for el in dict_methods_od_files[k] if el not in ["LOP_basic"]
            ]
...
def get_dictionaries_methods(ml="default", data_fold="data"):
    ...
    odbinodred_od_files = get_dict_methods_od(
        data_fold, method="OD-BINOD-RED", method_list=ml
    )
    red_files = get_dict_methods_red(data_fold, method_list=ml)
    odod_od_files = get_dict_methods_od(
        data_fold, method="OD-BINOD^OD-BINOD", method_list=ml
    )
    redod_od_files = get_dict_methods_od(
        data_fold, method="RED^OD-BINOD", method_list=ml
    )
    filelist = os.listdir(data_fold)
    for f in filelist:
        tmp1 = {
            "OD-BINOD-RED": odbinodred_od_files[f].copy(),
        ...
        tmp1["RED^OD-BINOD"] = [
            el for el in tmp1["RED^OD-BINOD"] if ("LOP_basic" not in el)
        ].copy()
        dict_method_od[f] = tmp1.copy()
        dict_method_red[f] = tmp2.copy()
    try:
        dict_method_red["kddcup-land_vs_portsweep"]["OD-BINOD^RED"] = [
            el
            for el in dict_method_red["kddcup-land_vs_portsweep"]["RED^RED"]
...
def check_columns(df_train, df_test, features):
    l1 = [
        col for col in df_train.columns.tolist() if col not in ["TARGET", "INDEX"]
    ].copy()
    l2 = [
        col for col in df_test.columns.tolist() if col not in ["TARGET", "INDEX"]
    ].copy()
    x1 = set(l1)
    x2 = set(l2)
    x3 = (x1 - x2).copy()
    ...
    features = [
        col for col in features.copy() if (col not in feats_train_not_in_test)
    ].copy()
    inf_mean_cols = (
        df_train[features]
...
def check_prescaler_df(df):
    ...
    df.dropna()
    inf_mean_cols = df.columns[np.where(np.mean(df) == np.inf)].tolist()
    inf_var_cols = df.columns[np.where(np.var(df) == np.inf)].tolist()
    features = [
        col
        for col in df.columns
        if (col not in inf_mean_cols) & (col not in inf_var_cols)
    ]
    return df[features].copy()


def get_whole_FROID_FULL(
    X_train,
    X_test,
    y_train,
    y_test,
    ...
    np.random.seed(42)
    random.seed(42)
    od_binod_red_path_train = "{}/{}/{}_{}_OD-BINOD-RED_train.csv".format(
        data_fold, filename, filename, meta
    )
    odod_path_train = "{}/{}/{}_{}_OD-BINOD^OD-BINOD_train.csv".format(
        data_fold, filename, filename, meta
    )
    redod_path_train = "{}/{}/{}_{}_RED^OD-BINOD_train.csv".format(
        data_fold, filename, filename, meta
    )
    redred_path_train = "{}/{}/{}_{}_RED^RED_train.csv".format(
        data_fold, filename, filename, meta
    )
    odred_path_train = "{}/{}/{}_{}_OD-BINOD^RED_train.csv".format(
        data_fold, filename, filename, meta
    )
    od_binod_red_path_test = "{}/{}/{}_{}_OD-BINOD-RED_test.csv".format(
        data_fold, filename, filename, meta
    )
    odod_path_test = "{}/{}/{}_{}_OD-BINOD^OD-BINOD_test.csv".format(
        data_fold, filename, filename, meta
    )
    redod_path_test = "{}/{}/{}_{}_RED^OD-BINOD_test.csv".format(
        data_fold, filename, filename, meta
    )
    redred_path_test = "{}/{}/{}_{}_RED^RED_test.csv".format(
        data_fold, filename, filename, meta
    )
    odred_path_test = "{}/{}/{}_{}_OD-BINOD^RED_test.csv".format(
        data_fold, filename, filename, meta
    )
    # here we prepare all the various FROID for different inceptions
    s1 = FROID(
        "OD-BINOD-RED",
        list_methods_od=dict_method_od[filename]["OD-BINOD-RED"],
        list_methods_red=dict_method_red[filename]["OD-BINOD-RED"],
    )
    s2_1 = FROID(
        "OD-BINOD",
        list_methods_od=dict_method_od[filename]["OD-BINOD^OD-BINOD"],
        list_methods_red=[],
        inception=True,
    ...
    s2_2 = FROID(
        "OD-BINOD",
        list_methods_od=dict_method_od[filename]["RED^OD-BINOD"],
        list_methods_red=[],
        inception=True,
        previous_stage="RED",
    )
    s2_3 = FROID(
        "RED",
        list_methods_od=[],
        list_methods_red=dict_method_red[filename]["RED^RED"],
        inception=True,
        previous_stage="RED",
    )
    s2_4 = FROID(
    ...
    if (os.path.exists(od_binod_red_path_test)) & (
        os.path.exists(od_binod_red_path_train)
    ):
        print("path exists!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        ...
        atts_s1 = [
            col
            for col in df_train_s1.columns
            if (col not in ["TARGET"]) & (col not in atts_level0)
        ]
        ...
        atts_s1_train = [
            col
            for col in X_train_s1.columns
            if (col not in atts_level0) & (col not in ["INDEX", "TARGET"])
        ]
        atts_s1_test = [
            col
            for col in X_test_s1.columns
            if (col not in atts_level0) & (col not in ["INDEX", "TARGET"])
        ]
        atts_s1 = sorted(
            list(set(atts_s1_train.copy()).intersection(set(atts_s1_test.copy()))),
            reverse=True,
        )
        ...
        df_train_s1.to_csv(
            "{}/{}/{}_{}_OD-BINOD-RED_train.csv".format(
                data_fold, filename, filename, meta
            ),
            index=False,
        )
        ...
        df_test_s1.to_csv(
            "{}/{}/{}_{}_OD-BINOD-RED_test.csv".format(
                data_fold, filename, filename, meta
            ),
            index=False,
        )
    ...
    atts_s2_od = [
        col
        for col in X_full_s1.columns
        if ("scores_" in col) & (col not in atts_level0)
    ]
    atts_s2_red = [
        col for col in X_full_s1.columns if ("comp_" in col) & (col not in atts_level0)
    ]
    ...
    if (os.path.exists(odod_path_test)) & (os.path.exists(odod_path_train)):
        ...
        atts_s2_1 = [
            col
            for col in df_train_s2_1.columns
            if (col not in ["TARGET"]) & (col not in atts_level0)
        ]
        try:
        ...
        try:
            ...
            raise ValueError("FROID FULL: s2_1.fit")
        ...
        atts_s2_1_train = [
            col
            for col in X_train_s2_1.columns
            if (col not in atts_s2_od) & (col not in ["INDEX", "TARGET"])
        ]
        atts_s2_1_test = [
            col
            for col in X_test_s2_1.columns
            if (col not in atts_s2_od) & (col not in ["INDEX", "TARGET"])
        ]
        atts_s2_1 = sorted(
            list(set(atts_s2_1_train.copy()).intersection(set(atts_s2_1_test.copy()))),
            reverse=True,
        )
        ...
        try:
            ...
            raise ValueError("FROID FULL: scaler_s2_1.fit did not work")
        try:
            X_train_s2_1[atts_s2_1_toscale] = scaler_s2_1.transform(
                X_train_s2_1[atts_s2_1_toscale]
            ).copy()
            X_test_s2_1[atts_s2_1_toscale] = scaler_s2_1.transform(
                X_test_s2_1[atts_s2_1_toscale]
            ).copy()
        except:
            raise ValueError("FROID FULL: scaler_s2_1.transform did not work")
        df_train_s2_1 = pd.concat([X_train_s2_1, y_train], axis=1)
        df_test_s2_1 = pd.concat([X_test_s2_1, y_test], axis=1)
        df_train_s2_1.to_csv(
            "{}/{}/{}_{}_OD-BINOD^OD-BINOD_train.csv".format(
                data_fold, filename, filename, meta
            ),
            index=False,
        )
        ...
        df_test_s2_1.to_csv(
            "{}/{}/{}_{}_OD-BINOD^OD-BINOD_test.csv".format(
                data_fold, filename, filename, meta
            ),
            index=False,
        )
    ...
    if (os.path.exists(redod_path_test)) & (os.path.exists(redod_path_train)):
        ...
        atts_s2_2 = [
            col
            for col in df_train_s2_2.columns
            if (col not in ["TARGET", "INDEX"]) & (col not in atts_level0)
        ]
        ...
        atts_s2_2_train = [
            col
            for col in X_train_s2_2.columns
            if (col not in atts_s2_red) & (col not in ["INDEX", "TARGET"])
        ]
        atts_s2_2_test = [
            col
            for col in X_test_s2_2.columns
            if (col not in atts_s2_red) & (col not in ["INDEX", "TARGET"])
        ]
        atts_s2_2 = sorted(
            list(set(atts_s2_2_train.copy()).intersection(set(atts_s2_2_test.copy()))),
            reverse=True,
        )
        atts_s2_2_toscale = [el for el in atts_s2_2 if ("bin_score" not in el)]
        scaler_s2_2.fit(X_train_s2_2[atts_s2_2_toscale])
        X_train_s2_2[atts_s2_2_toscale] = scaler_s2_2.transform(
            X_train_s2_2[atts_s2_2_toscale]
        ).copy()
        X_test_s2_2[atts_s2_2_toscale] = scaler_s2_2.transform(
            X_test_s2_2[atts_s2_2_toscale]
        ).copy()
        df_train_s2_2 = pd.concat([X_train_s2_2, y_train], axis=1)
        df_test_s2_2 = pd.concat([X_test_s2_2, y_test], axis=1)
        df_train_s2_2.to_csv(
            "{}/{}/{}_{}_RED^OD-BINOD_train.csv".format(
                data_fold, filename, filename, meta
            ),
            index=False,
        )
        df_test_s2_2.to_csv(
            "{}/{}/{}_{}_RED^OD-BINOD_test.csv".format(
                data_fold, filename, filename, meta
            ),
            index=False,
        )
    ...
    if (os.path.exists(redred_path_test)) & (os.path.exists(redred_path_train)):
        ...
        atts_s2_3 = [
            col
            for col in df_train_s2_3.columns
            if (col not in ["TARGET", "INDEX"]) & (col not in atts_level0)
        ]
        ...
        atts_s2_3_train = [
            col
            for col in X_train_s2_3.columns
            if (col not in atts_s2_red) & (col not in ["INDEX", "TARGET"])
        ]
        atts_s2_3_test = [
            col
            for col in X_test_s2_3.columns
            if (col not in atts_s2_red) & (col not in ["INDEX", "TARGET"])
        ]
        atts_s2_3 = sorted(
            list(set(atts_s2_3_train.copy()).intersection(set(atts_s2_3_test.copy()))),
            reverse=True,
        )
        atts_s2_3_toscale = [el for el in atts_s2_3 if ("bin_score" not in el)]
        scaler_s2_3.fit(X_train_s2_3[atts_s2_3_toscale])
        X_train_s2_3[atts_s2_3_toscale] = scaler_s2_3.transform(
            X_train_s2_3[atts_s2_3_toscale]
        ).copy()
        X_test_s2_3[atts_s2_3_toscale] = scaler_s2_3.transform(
            X_test_s2_3[atts_s2_3_toscale]
        ).copy()
        df_train_s2_3 = pd.concat([X_train_s2_3, y_train], axis=1)
        df_test_s2_3 = pd.concat([X_test_s2_3, y_test], axis=1)
        df_train_s2_3.to_csv(
            "{}/{}/{}_{}_RED^RED_train.csv".format(data_fold, filename, filename, meta),
            index=False,
        )
        df_test_s2_3.to_csv(
            "{}/{}/{}_{}_RED^RED_test.csv".format(data_fold, filename, filename, meta),
            index=False,
        )
    # OD-BINOD^RED
    if (os.path.exists(odred_path_test)) & (os.path.exists(odred_path_train)):
        ...
        atts_s2_4 = [
            col
            for col in df_train_s2_4.columns
            if (col not in ["TARGET"]) & (col not in atts_level0)
        ]
        ...
        atts_s2_4_train = [
            col
            for col in X_train_s2_4.columns
            if (col not in atts_s2_od) & (col not in ["INDEX", "TARGET"])
        ]
        atts_s2_4_test = [
            col
            for col in X_test_s2_4.columns
            if (col not in atts_s2_od) & (col not in ["INDEX", "TARGET"])
        ]
        atts_s2_4 = sorted(
            list(set(atts_s2_4_train.copy()).intersection(set(atts_s2_4_test.copy()))),
            reverse=True,
        )
        atts_s2_4_toscale = [el for el in atts_s2_4 if ("bin_score" not in el)]
        scaler_s2_4.fit(X_train_s2_4[atts_s2_4_toscale])
        try:
            X_train_s2_4[atts_s2_4_toscale] = scaler_s2_4.transform(
                X_train_s2_4[atts_s2_4_toscale]
            ).copy()
            X_test_s2_4[atts_s2_4_toscale] = scaler_s2_4.transform(
                X_test_s2_4[atts_s2_4_toscale]
            ).copy()
        except:
            raise ValueError("FROID FULL: scaler_s2_4.transform did not work")
        df_train_s2_4 = pd.concat([X_train_s2_4, y_train], axis=1)
        df_test_s2_4 = pd.concat([X_test_s2_4, y_test], axis=1)
        df_train_s2_4.to_csv(
            "{}/{}/{}_{}_OD-BINOD^RED_train.csv".format(
                data_fold, filename, filename, meta
            ),
            index=False,
    ...
    atts_odod_train = [
        col for col in X_train_s2_1.columns.tolist() if (col not in atts_level1)
    ].copy()
    atts_redod_train = [
        col for col in X_train_s2_2.columns.tolist() if (col not in atts_level1)
    ].copy()
    atts_redred_train = [
        col for col in X_train_s2_3.columns.tolist() if (col not in atts_level1)
    ].copy()
    atts_odred_train = [
        col for col in X_train_s2_4.columns.tolist() if (col not in atts_level1)
    ].copy()
    if index_train is None:
        db_train = pd.concat(
            [
                X_train,
                X_train_s1[atts_level1],
                X_train_s2_1[atts_odod_train],
                X_train_s2_2[atts_redod_train],
                X_train_s2_3[atts_redred_train],
                X_train_s2_4[atts_odred_train],
                y_train,
            ],
            axis=1,
        )
    ...
    db_train.to_csv(
        "{}/{}/{}_{}_WHOLE_train.csv".format(data_fold, filename, filename, meta),
        index=False,
    )
    # WHOLE DATASET TEST
    atts_odod_test = [
        col for col in X_test_s2_1.columns.tolist() if (col not in atts_level1)
    ].copy()
    atts_redod_test = [
        col for col in X_test_s2_2.columns.tolist() if (col not in atts_level1)
    ].copy()
    atts_redred_test = [
        col for col in X_test_s2_3.columns.tolist() if (col not in atts_level1)
    ].copy()
    atts_odred_test = [
        col for col in X_test_s2_4.columns.tolist() if (col not in atts_level1)
    ].copy()
    if index_test is None:
        db_test = pd.concat(
            [
                X_test,
                X_test_s1[atts_level1],
                X_test_s2_1[atts_odod_test],
                X_test_s2_2[atts_redod_test],
                X_test_s2_3[atts_redred_test],
                X_test_s2_4[atts_odred_test],
                y_test,
            ],
            axis=1,
        )
    ...
    print(db_train.head())
    db_test.to_csv(
        "{}/{}/{}_{}_WHOLE_test.csv".format(data_fold, filename, filename, meta),
        index=False,
    )
    ...
    time_df.to_csv(
        "{}/{}/{}_{}_time.csv".format(data_fold, filename, filename, meta), index=False
    )
    return db_train, db_test


def experiment_FROID(
    filename,
    classifier="lgbm",
    method="WHOLE",
    feat_sel="nofs",
    ...
    if methodlist != "default":
        ...
        print(meta)
    path_train = "{}/{}/{}_{}_WHOLE_train.csv".format(
        data_fold, filename, filename, meta
    )
    ...
    if (os.path.exists(path_train)) & (os.path.exists(path_test)):
        ...
        features = [
            col for col in X_train.columns.tolist() if (col not in ["TARGET", "INDEX"])
        ]
        print("experiment FROID methodlist is {}".format(methodlist))
        dict_method_od, dict_method_red = get_dictionaries_methods(
            ml=methodlist, data_fold=data_fold
        )
        print(dict_method_od[filename])
        # print(dict_method_red)
        # print(dict_method_od)
        print(filename)
        # we run the preprocessing of FROID here
        df_train, df_test = get_whole_FROID_FULL(
            X_train[features],
            X_test[features],
            y_train,
            y_test,
    ...
    whole_cols = [
        col
        for col in df_train.columns.tolist()
        if (col not in ["TARGET", "INDEX"]) & ("LDA" not in col)
    ]
    if method == "WHOLE":
        ...
        features = (
            orig_cols
            + [col for col in whole_cols if ("ORIG" in col) & ("LDA" not in col)].copy()
        )
    elif method == "XGBOD":
        features = (
            orig_cols
            + [
                ...
                if ("ORIG" in col) & ("scores_" in col) & ("bin_" not in col)
            ].copy()
        )
    if classifier == "lgbm":
    ...
    if params == "ht":
        ...
        if classifier == "lgbm":
            clf_base = lgb.LGBMClassifier(
                importance_type="gain", random_state=42, n_jobs=4, **best_params
            )
    if calibration == "calib":
        clf_cal = CalibratedClassifierCV(
            base_estimator=clf_base, cv=5, method="isotonic"
        )
        feat_imp = False
    if feat_sel == "nofs":
        if calibration == "calib":
        ...
        inf_mean_cols = df_train.columns[np.where(np.mean(df_train) == np.inf)].tolist()
        inf_var_cols = df_train.columns[np.where(np.var(df_train) == np.inf)].tolist()
        features = [
            el
            for el in features
        ...
        print(calibration)
    if (classifier in ["dt", "xgb", "rf"]) & (
        (len(np.where(df_train.max() > 10**20)[0]) > 0)
    ...
    yy = pd.DataFrame(np.c_[y_true, preds, scores], columns=["true", "preds", "scores"])
    results = pd.DataFrame()
    ...
    results = pd.concat([results, tmp], axis=0)
    results_path = "{}/{}/{}_{}_{}_{}_{}_{}_{}.csv".format(
        results_fold,
        filename,
        filename,
        meta,
    ...
    if (feat_imp == True) & (classifier in ["lgbm", "xgb", "cat"]):
        if isinstance(clf, Pipeline):
            ...
            cals = [
                clf.calibrated_classifiers_[i].base_estimator.feature_importances_
                for i in range(len(clf.calibrated_classifiers_))
            ]
            col_fi = ["features"] + [
                "feat_importance_{}".format(i)
                for i in range(len(clf.calibrated_classifiers_))
            ]
            tt = np.column_stack(
                (clf.calibrated_classifiers_[0].base_estimator.feature_name_, cals)
            )

            try:
                tmp1 = pd.DataFrame(tt, columns=col_fi)
                print("there we go")
            except:
                tmp1 = pd.DataFrame(
                    [["nocol", 0]], columns=["features", "feat_importance"]
                )
        else:
            try:
                if classifier == "lgbm":
        ...
        tmp1.to_csv(
            "{}/{}/FEAT_IMP_{}_{}_{}_{}_{}_{}_{}.csv".format(
                results_fold,
                filename,
                filename,
                meta,
...
def experiment_NOFROID(
    ...
    if meta == "CURE":
        ...
        string_meta = meta
    train_path = "{}/{}/{}_{}_train.csv".format(
        data_fold, filename, filename, string_meta
    )
    if os.path.exists(train_path):
        ...
        if os.path.exists("final_results/{}".format(filename)) == False:
            ...
            results.to_csv(
                "final_results/{}/results_{}_{}_{}_{}_{}_{}.csv".format(
                    filename,
                    filename,
                    classifier,
                    meta,
        ...
        clf.fit(df_train[features], df_train["TARGET"])
        end_time = time.time()
        tmp_time = pd.DataFrame(
            [[filename, end_time - start_time, meta]],
            columns=[filename, "time", "meta"],
        )
        if binary:
            scores = clf.predict_proba(df_test[features])[:, 1]
            preds = clf.predict(df_test[features])
        ...
        if os.path.exists("final_results/{}".format(filename)) == False:
            ...
            tmp_time.to_csv(
                "final_results/{}/{}_{}_time.csv".format(filename, filename, meta)
            )
        else:
            results.to_csv(
                "final_results/{}/results_{}_{}_{}_{}_{}_{}.csv".format(
